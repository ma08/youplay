...
class CursesMenu(object):
    ...
    #event_change_run currently is run for every draw. TODO something efficient
    #input run when use presses something other than the current functional inputs so that 
    #the app can take the input and do something TODO
    def __init__(self, title=None, subtitle=None, show_exit_option=True,event_change_run=None,event_change_args=None,input_run=None,input_run_args=None,process_query=None):
        """
        ...
        self.input_run_args=input_run_args
        self.event_change_run=event_change_run
        self.event_change_args=event_change_args
    ...
    def esc_is_terminate(self,x):
        if x==27: #Escape
            self.exited_search_by_escape=True
            return 7
        self.exited_search_by_escape=False

        return x



    def go_to_search_box(self):
        # self.search_box.box()
        # tb.stripspaces=1
        curses.curs_set(1)
        self.search_box.do_command(11) #CTRL-K ASCII

        self.exited_search_by_escape=False

        text = self.search_box.edit(validate=self.esc_is_terminate)
        text=self.search_box.gather()

        f1=open('./testfile', 'w+')
        print >> f1, text 
        print >> f1, "   ----       " 

        if(not self.exited_search_by_escape):
            self.remove_all_items()
            self.process_query(*[self,text])

    def _main_loop(self, scr):
        if scr is not None:
            CursesMenu.stdscr = scr
        ##CHANGE newpad to newwin
        self.screen = curses.newwin(len(self.items) + 6, int(CursesMenu.stdscr.getmaxyx()[1]/1.5))
        ...
        win = curses.newwin(1, 60, CursesMenu.stdscr.getmaxyx()[0]-1, 0)
        # win.addstr(0,10,"Enter Search: ")
        win.addstr(0,0,"Search String:")
        win.refresh()
        # stdscr.refresh()
        win.border()
        text_win = curses.newwin(1, 60, CursesMenu.stdscr.getmaxyx()[0]-1, len("Search String")+1)
        self.search_win = text_win
        self.search_box = curses.textpad.Textbox(text_win,insert_mode=True)
    ...
    def draw(self):
        ...
        if(not self.event_change_run is None):
            self.event_change_run(*self.event_change_args)
    ...
    def process_user_input(self):
        ...
        if ord('1') <= user_input <= go_to_max:
            ...
            self.go_up()
        elif user_input == ord('q'): #TODO more efficient way
            self.go_to(len(self.items)-1)
            self.select()
